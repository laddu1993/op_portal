...
export class PartsComponent implements OnInit, AfterViewInit {  
    ...
    this.service.login().subscribe(
      (res: any) => {
        if (res && res.token) {
          ...
          // Call loadAgentEmail after successful login
          this.loadAgentEmail();
        } else {
  ...
  ngOnInit(): void {
    ...
    this.searchForm = this.fb.group({
      selectedProduct: ['all'], // Default value is "all"
    });
  }
    ...
    this.service.getAgentEmail().subscribe(
      (res: any) => {
        if (res.status === 'success') {
          ...
          // Once the email is loaded, initialize the rest of the page
          this.initializePage();
          ...
          this.router.navigate(['/contact-admin']);  // Redirect to Contact Admin page
        }
      },
      (error) => {
        console.error('Error occurred while loading agent email:', error);
        // Check for 422 status code and redirect to 404
        if (error.status === 422) {
          this.router.navigate(['/404']);  // Redirect to 404 page
        } else {
          this.router.navigate(['/contact-admin']);  // Redirect to Contact Admin for other errors
        }
      }
    );
  }
